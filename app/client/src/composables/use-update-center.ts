...
const state = reactive({
  supported: typeof window !== 'undefined' ? Boolean(window.mfsender?.updates) : false,
...
const initListeners = () => {
  const updates = window.mfsender?.updates;
  if (!updates) {
...
export const useUpdateCenter = () => {
  ...
  const checkForUpdates = () => window.mfsender?.updates?.checkForUpdates();
  const downloadAndInstall = () => window.mfsender?.updates?.downloadUpdate({ install: true });
  const downloadOnly = () => window.mfsender?.updates?.downloadUpdate({ install: false });
