...
const REPO = 'mfsender';
const USER_AGENT = 'mfsender/auto-updater';
...
const DEFAULT_DISABLE_INSTALL = process.env.MFSENDER_UPDATE_DISABLE_INSTALL === 'true'
  ? true
  : (process.env.MFSENDER_UPDATE_DISABLE_INSTALL === 'false'
      ? false
...
class UpdateManager {
  constructor({ mainWindow }) {
    ...
    this.mode = process.env.MFSENDER_UPDATE_MODE
      ? String(process.env.MFSENDER_UPDATE_MODE).toLowerCase()
      : (app.isPackaged ? UPDATE_CHANNELS.STABLE : UPDATE_CHANNELS.DEV);

    this.allowPrerelease = process.env.MFSENDER_UPDATE_ALLOW_PRERELEASE === 'true';
    this.feedUrl = process.env.MFSENDER_UPDATE_FEED_URL || null;
  ...
  async handleDevDownload({ installOnComplete }) {
    ...
    try {
      const downloadDir = process.env.MFSENDER_UPDATE_DOWNLOAD_DIR || app.getPath('downloads');
...
export const scheduleInitialUpdateCheck = (delayMs = 10000) => {
  ...
  const scheduleDelay = Number.parseInt(process.env.MFSENDER_UPDATE_INITIAL_DELAY_MS || `${delayMs}`, 10);
