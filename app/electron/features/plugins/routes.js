...
const uploadTempDir = path.join(os.tmpdir(), 'mfsender-plugins');
...
async function readAndValidateManifest(manifestPath, appVersion = null) {
  ...
  if (appVersion && manifest.minAppVersion) {
    if (compareVersions(manifest.minAppVersion, appVersion) > 0) {
      throw new Error(
        `Plugin "${manifest.name}" requires mfsender v${manifest.minAppVersion} or higher. ` +
        `Current version is v${appVersion}. Please update mfsender before installing this plugin.`
      );
    }
  }
...
export function createPluginRoutes({ getClientWebSocket, broadcast } = {}) {
  ...
  router.post('/install', upload.single('plugin'), async (req, res) => {
    try {
      ...
      const extractDir = path.join(os.tmpdir(), 'mfsender-plugin-extract', Date.now().toString());
    ...
    try {
      ...
      try {
        ...
        const response = await fetchFn(url, {
          redirect: 'follow',
          headers: {
            'User-Agent': 'mfsender-Plugin-Installer'
          }
        });
  ...
  router.get('/:pluginId/check-update', asyncHandler(async (req, res) => {
    ...
    try {
      ...
      const headers = {
        'User-Agent': 'mfsender-Plugin-Manager',
