...
export class CommandProcessor {
  ...
  async processNonM98(command, context = {}) {
    ...
    // Handle $MFSENDER_CLEAR_MSG command - clear plugin message and notify all clients
    if (command.trim().toUpperCase() === '$MFSENDER_CLEAR_MSG') {
      log('$MFSENDER_CLEAR_MSG command detected - clearing plugin message');
      ...
      this.broadcast('cnc-command', {
        id: commandId,
        command: '$MFSENDER_CLEAR_MSG',
        displayCommand: '$MFSENDER_CLEAR_MSG',
        status: 'pending',
