# mfsender Plugin Development Guide

## Overview

mfsender supports a plugin system that allows developers to extend functionality without modifying the core application. Plugins can hook into the G-code job lifecycle, modify G-code on the fly, and interact with the CNC controller.
...
- **macOS**: `~/Library/Application Support/mfsender/plugins/`
- **Windows**: `%APPDATA%\mfsender\plugins\`
- **Linux**: `~/.config/mfsender/plugins/`
...
- **minAppVersion**: Minimum mfsender version required (e.g., "0.3.0"). Installation will be blocked if the app version is lower than this requirement
...
The context object provides access to mfsender APIs:
...
mfsender organizes plugins into categories based on their function:
...
Called when a G-code file is loaded or uploaded into mfsender. This is the primary event for post-processor plugins that transform G-code files.
...
mfsender includes a CLI tool for plugin development:
...
Creates `plugins/com.mfsender.my-plugin.zip`

### Install a Plugin

Install a built plugin to mfsender:
...
# Reload the plugin without restarting mfsender
curl -X POST http://localhost:8090/api/plugins/com.mfsender.my-plugin/reload
```
...
Check plugin logs in the main mfsender console output. Look for messages prefixed with `[PLUGIN:your-plugin-id]`.
