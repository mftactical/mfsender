...
If you enjoy **mfsender** and my other CNC / maker projects, consider supporting me!
...
- **macOS**: open the `.dmg`, drag mfsender to Applications. Since the app is not code-signed by Apple, you'll need to clear the quarantine attribute before running:
  ```bash
  xattr -c /Applications/mfsender.app
  ```
  Open Terminal (Applications → Utilities → Terminal), paste the command above, and press Enter. After that, you can open mfsender normally.
...
Run the embedded server without the Electron window and access mfsender from any browser on your network.

- **macOS**: `/Applications/mfsender.app/Contents/MacOS/mfsender --headless`
- **Windows**: `"C:\\Program Files\\mfsender\\mfsender.exe" --headless`
- **Linux/Raspberry Pi (AppImage)**: `./mfsender-*.AppImage --headless` or `mfsender --headless` if installed in PATH
...
## 💾 Data & Settings
mfsender stores its data per‑platform:
- **macOS**: `~/Library/Application Support/mfsender/`
- **Windows**: `%APPDATA%/mfsender/`
- **Linux / Raspberry Pi**: `~/.config/mfsender/`
...
## 🔒 Privacy
Everything runs locally. mfsender does not send your files or machine data to remote servers.
...
## 👨‍💻 For Developers
Looking to build, contribute, or extend mfsender? See `docs/DEVELOPER_GUIDE.md` for:
...
mfsender is available under a **dual license**:

### Open Source License (GPL-3.0)
For open source projects and personal use, mfsender is licensed under the [GNU General Public License v3.0](LICENSE-GPL-3.0).
